...
The image doubles as a throwaway agent sandbox via `ai_sandbox.sh`, which lives in [devopscoop/scripts](https://github.com/devopscoop/scripts) (it was this repo's `sandbox.sh` until August 2026): Claude Code or OpenCode runs as your own UID/GID with only `$PWD` mounted (at `/work/<dirname>`), a shared `~/.sandbox-home` as the container HOME (authenticate once across projects; deliberately not the host's real `~/.claude`), and no host credentials or Docker socket. The script's knobs (`IMAGE`, `DOCKER`, `SANDBOX_HOME`, `ANTHROPIC_API_KEY`) and design rationale are documented in that repo's README and AGENTS.md — keep the two repos consistent when the image's Claude Code install or the script's mount contract changes.
