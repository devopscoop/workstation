...
`sandbox.sh` runs Claude Code or OpenCode inside the image as a throwaway sandbox: your own UID/GID, only `$PWD` mounted (as `/work/<dirname>`), no `~/.aws`/`~/.kube`/`~/.ssh`/`~/.gnupg`, and no Docker socket. Agent state (auth, config, history) lives in one shared host dir mounted as the container HOME — `~/.sandbox-home` by default — so you authenticate once, not per project. It is deliberately **not** the host's real `~/.claude`: the sandbox must never write host config (sandbox settings hooks would execute on the host), and macOS keeps the OAuth token in the Keychain anyway, so mounting the real config wouldn't even carry auth. The project mounts at `/work/<dirname>` rather than a fixed `/work` because Claude keys per-project state on the path — a fixed mount point would collapse every project onto one key. Network is open — only the filesystem is sandboxed. Knobs: `WORKSTATION_IMAGE`, `DOCKER` (defaults to `docker` on macOS, `sudo docker` on Linux), `SANDBOX_HOME` (point at `$PWD/.sandbox-home` to isolate one project's agent state), `ANTHROPIC_API_KEY` (forwarded as an env var, never a mounted file).
